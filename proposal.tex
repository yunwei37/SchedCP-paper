...
We will implement an extensible benchmark framework with three components: 1) a workload runner for executing applications and measuring performance, 2) a secure agent execution environment with a rich toolset for observation and optimization, and 3) an evaluation harness to automate agent scoring against SLOs. We will use the benchmark to evaluate several agents and models(e.g. qwen, GPT, claude, gemini), demonstrating its ability to differentiate their capabilities and guide future research.
