...
\begin{abstract}
Operating system schedulers suffer from a fundamental semantic gap, where kernel policies fail to understand application-specific needs, leading to suboptimal performance. We introduce \sys, the first framework that enables autonomous Large Language Model (LLM) agents to safely and efficiently optimize Linux schedulers. Our core insight is to architect a decoupled control plane that separates the AI's role of semantic reasoning ("what to optimize") from the system's role of safe execution ("how to observe and act"). SchedCP provides a stable interface with three key services: a Workload Analysis Engine for adaptive context provisioning, an evolving Scheduler Policy Repository that reduces cost and latency by reusing proven solutions, and a rigorous Execution Verifier that sandboxes and validates all AI-generated code before deployment. We demonstrate this architecture's power with sched-agent, a multi-agent system that autonomously analyzes workloads, synthesizes custom eBPF scheduling policies, and deploys them via the sched\_ext infrastructure. Our evaluation shows that SchedCP achieves up to an 80\% performance improvement in kernel compilation, a 50\% latency reduction on scheduler benchmarks, and an 80\% cost reduction compared to naive agentic approaches, all while maintaining high success rate. By bridging the semantic gap, SchedCP democratizes expert-level system optimization and represents a step towards creating truly self-optimizing, application-aware operating systems.
\end{abstract}
